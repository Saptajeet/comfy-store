  ...
  return (
    <article key={cartID} className="mb-12 flex flex-col gap-y-4 sm:flex-row flex-wrap border-b border-base-300 pb-6 last:border-b-0">
        ...
        <div className="sm:ml-12">
            ...
            {/* REMOVE */}
            <button className="mt-2 link link-primary link-hover text-sm uppercase" onClick={removeItemFromCart}>remove</button>
        </div>
        {/* PRICE */}
        <p className="font-medium sm:ml-auto">{formatPrice(price)}</p> {/*ml-auto puts it in the rightmost place or the farthest from left*/}
    </article>
  )
}
